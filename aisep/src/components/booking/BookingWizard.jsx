...
export default function BookingWizard({ onClose, user, initialAdvisorId = null, initialProjectId = null, sourceBookingId = null }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const options = await bookingService.getAdvisorOptions(selectedProject.projectId);
    ...
    <div className={styles.overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className={styles.modal}>
        ...
        <div className={styles.body}>
          ...
          {step === 1 && (
            <div className={styles.stepContent}>
              <p className={styles.stepHint}>Cố vấn được phân công cho dự án <strong>{selectedProject?.projectName}</strong></p>
              {advisorsLoading && (
                <div className={styles.loadingState}>
                  <Loader size={24} className={styles.spin} />
              ...
              {!advisorsLoading && !advisorsError && projectAdvisorsCount === 0 && (
              ...
              {!advisorsLoading && !advisorsError && advisorOptions.length === 0 && (
                <div className={styles.emptyState}>
                  <User size={40} />
                  <p>Dự án này chưa có cố vấn được phân công.</p>
                </div>
              )}
